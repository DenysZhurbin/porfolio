...
import { microModalConfig } from '@/modules/containers/modal';
...
export const startLoading = () => {
  ...
  MicroModal.show(modalLoaderId);
};

export const finishLoading = () => {
  const modal = $(`#${modalLoaderId}`);

  MicroModal.close(modalLoaderId, {
    onClose() {
...
export const loadAssets = (absolutePath, relativePaths = [], callback) => {
  let loadedScripts = 0;
  const itemsToLoad = relativePaths.length;
  const urls = relativePaths.map(path => {
    return `${absolutePath}${path}`
  });
  const attemptToRunCallback = () => {
    if (loadedScripts === itemsToLoad) {
